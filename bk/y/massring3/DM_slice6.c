int Dipole6(struct dipole d, double dp, double vin[2][3], double vout[2][3],
            int k0, int k1,
...
//  dz =dz;
  ...
  for(k=k0;k<k1;k++){
    ...
    GetExitFrin(k, x, y, z, xt, yt, zt, rad, dlen,
...
void GetExitFrin(int k, double x, double y, double z,
		 double xt, double yt, double zt, double rad, double dlen,
		 double  Bxa[Z_SLICE+1][R_SLICE+1][N_SLICE+1],
		 double Bxbx[Z_SLICE+1][R_SLICE+1][N_SLICE+1],
		 double Bxcx[Z_SLICE+1][R_SLICE+1][N_SLICE+1],
		 double Bxdx[Z_SLICE+1][R_SLICE+1][N_SLICE+1],
  ...
  double px, py, pz, p, p0, q, q0, r, rad_t;
  double fx,fy,fz,xc,yc,zc;
  double gx,gy,gz,ca,cb,cc;
  ...
  if(rad_t<5.e+9){
	  ...
	  ca = gy*gy+gz*gz;
	  cb = 2.*gx*gy*(dlen-xc);
	  cc = (gx*gx+gz*gz)*(dlen-xc)*(dlen-xc)-gz*gz*rad_t*rad_t;
	  *xe = dlen;
	  *ye = (-cb+(gz/fabs(gz))*sqrt(cb*cb-4.*ca*cc))/2./ca+yc;
	  *ze = -((*xe-xc)*gx+(*ye-yc)*gy)/gz+zc;
      }
      else{
	  ca = gy*gy+gz*gz;
	  cb = 2.*gx*gz*(dlen-xc);
	  cc = (gx*gx+gy*gy)*(dlen-xc)*(dlen-xc)-gy*gy*rad_t*rad_t;
	  *xe = dlen;
	  *ze = (-cb-(gy/fabs(gy))*sqrt(cb*cb-4.*ca*cc))/2./ca+zc;
	  *ye = -((*xe-xc)*gx+(*ze-zc)*gz)/gy+yc;
      }
