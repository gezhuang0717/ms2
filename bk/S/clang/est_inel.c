/* Estimate inelastic events with dp/p histgram [2015/03/04 S.Suzuki] */
...
void fitfl(double flini,double *fl,double *offset);
double flfunc(double);

/* shared variables */
double *d57,*cfit,*csim,*csim2,aa[NCF],sa[NCF];
int limit,bin57,np1;
double *eex;
double slope,exex,ein,emin,emax,sig_exp,ave,sig,thtgt; /* for using in func */
...
int main (void) {
  ...
  double cexp_max,cexp_l,cexp_h,fwhm_exp,thick_cor,flini=0.002,fl,eoffset;
...
/* fit width of simulation data to exp. data and calculate energy offset */
  printf("calculate fluctuation of the beam to fit to Exp. data...\n");
  fitfl(flini,&fl,&eoffset);   /* fit fluctuation of the beam */
  printf("corrected fluctuation = %lf\n", fl);
...
//  exit(0);
  ...
  sim_(&A,&Z,&isotp,&cf1[1],&thickt,&fl,&ex2,&nevent,&ec,eout); /* simulatie Eout fortran */
  for(j=0;j<bin57;j++) csim[j]=0.;    /* initialize vector */
...
//  exit(0);
  ...
  sim_(&A,&Z,&isotp,&cf1[1],&thickt,&fl,&ex2,&nevent,&ec,eout); /* simulate Eout by fortran */
...
void fitfl(double flini,double *fl,double *offset) {
/* determine fluctuation of the beam to fit sim. to exp. data */
  double ax,bx,cx,fa,fb,fc,tol,xmin;
  bx=flini;
  ax=0.5*bx;
  cx=2.*bx;
  tol=1.e-4;
  mnbrak(&ax,&bx,&cx,&fa,&fb,&fc,flfunc);
  golden(ax,bx,cx,flfunc,tol,&xmin);
  *(fl)=xmin;  /* corrected fluctuation of the beam */
  printf("sig(sim)=%lf\n", sig);
  flfunc(xmin);
...
double flfunc(double xx) {
/* return difference of standerd deviation */
  double *eout,*counts,cmax,fl;
  int i,j,jl,jh;
  fl=fabs(xx);
  eout=dvector(0,neve);
  counts=dvector(0,bin57);
  sim_(&mass,&charge,&istp,&slope,&thtgt,&fl,&exex,&neve,&ein,eout); /* simulation by fortran program */
  ave=0.;
