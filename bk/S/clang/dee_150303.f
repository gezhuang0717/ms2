      Subroutine SIM(Ai,Zi,isotp,amp,thtgt,ex,count,beame,eout)
...
*      COMMON /FILE/NFILE,RFILE,DABLK
      ...
      Real*8 beame,eout(count),thtgt,ex
...
c      if(isotp.eq.1) then
      if(isotp.eq.1.or.isotp.eq.2) then

         xx = 1.
      elseif(isotp.eq.3) then
 77      xxp = urand2(IX)
         reratep=0.94296+0.052103*xxp+0.0048666*xxp**2

         xx = urand2(IX)
         yy=xx

         if(yy.le.reratep) then
            xx = xxp
         else
            goto 77
         endif
         ...
         xx = urand2(IX)
c      write(6,*)'********',deprj,matnam(1),thtgtr,'***********'

      endif

      thtgtr = thtgt * (1.-xx)
c      einc=(1+0.0233*(0.5-urand2(IX)))*beame
ccc modified by s.s
ccc       sigeinc = beame*0.00283
c       sigeinc = beame*0.0022
       sigeinc = beame*0.0021
...
c         a = Apara(II(1))
         a = amp
cmg24
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.056)
cmg25
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.0234)
cmg26
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.0695)
cmg27
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.036)
cmg28
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.053)
cmg29
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.019)
cmg30
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.049)
cmg31c60
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.0016)
cmg32c60
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.028)
cmg33c45
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.0015)
cmg34c9
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.026)
cmg35c9
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.0014)
cmg36c17
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.02458)
cmg37c17
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.0014)
cmg38c111
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-0.0232)

c s.s
c         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-4.439/A0)
         efrg  = (einc-deprj+(dlog(1.d0-urand2(IX)))/a-ex/A0)

c  first exited state(12c 4.439,9Be 1.665,27Al 0.884)    

*         write(6,*)' efrag',efrg
*         if(iii.eq.1)write(6,*)' iii,a,efrg ',iii,a,efrg
*         efrg  = (einc-deprj+(dlog(1.-urand2(IX)))/a-0.07033)
c      efrg = einc-deprj
c      efrg  = (einc-(einc-deprj)*urand2(IX)*urand2(IX)-deprj)
      ...
      elseif(AI.ne.A0 .and. AI.gt.A0) then
         ...
         deprj = ELOSS(einc,Z0,A0,MATNAM(1),thtgt*xx/10.)
         efrg = einc - deprj ! energy per nucleon
cc         write(*,*) einc
      endif
...
*         write(6,*)'sigmom,efrg : ',sigmom,efrg
      endif

c      write(6,*)' efrg after the target= ',efrg
...
******************************
      SUBROUTINE INIT
C     1、anafileから、bin数、DATAの最大値、最小値を読み込み、
C        メモリ(PAWC)内にヒストグラムの領域を確保する。
C     2、anafileから、gateに関する情報を抽出し、
C        GID,HID,LOGIDにその情報を書き込む。
******************************
      Implicit Real*8 (A-H,O-Z)
      COMMON /gat2D/XP(20,50),YP(20,50),NPNT(20)
      COMMON /RAW/RAW,IDATA
